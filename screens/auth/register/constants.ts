...
export const DAYS_OPTIONS = Array.from({ length: 31 }, (_, i) => (i + 1).toString().padStart(2, '0'));
export const MONTHS_OPTIONS = Object.keys(MONTH_MAP);

const currentYearNum = new Date().getFullYear();
export const CANDIDATE_YEARS_OPTIONS = Array.from(
  { length: currentYearNum - 1920 + 1 },
  (_, i) => (currentYearNum - i).toString()
);

export const COMPANY_YEARS_OPTIONS = Array.from(
  { length: currentYearNum - 1800 + 1 },
  (_, i) => (currentYearNum - i).toString()
);
