...
interface AiAssessmentModalProps {
  ...
  onClose: () => void;
  profession: string;
  onComplete: (tier: string, score: number, feedback: string, autogeneratedBio: string, recommendedSkills: string[]) => void;
}
...
export const AiAssessmentModal = ({
  ...
  const getAiRecommendedSkills = (profession: string, ans1: string, ans2: string): string[] => {
    ...
    if (lowerProf.includes('program') || lowerProf.includes('desarroll') || lowerProf.includes('tech') || lowerProf.includes('ingeni')) {
      skills.push('Arquitectura Limpia');
      if (skills.length < 2) skills.push('Análisis de Sistemas');
    } else if (lowerProf.includes('abogado') || lowerProf.includes('derecho') || lowerProf.includes('legal')) {
      skills.push('Análisis Regulatorio');
      if (skills.length < 2) skills.push('Pensamiento Analítico');
    } else if (lowerProf.includes('cocina') || lowerProf.includes('chef') || lowerProf.includes('gastrono')) {
      skills.push('Eficiencia Operativa');
      if (skills.length < 2) skills.push('Control de Calidad');
    } else if (lowerProf.includes('medic') || lowerProf.includes('salud') || lowerProf.includes('enfermer')) {
  ...
  const runGrading = () => {
    ...
    if (firstQ.startsWith('tech_')) {
      specializedKeywords = ['lazy', 'profile', 'memoria', 'clean', 'mvvm', 'microservicios', 'api', 'graphql', 'aws', 'gcp', 'cicd', 'pipeline', 'git', 'patrón', 'hilo', 'bloqueo', 'virtual', 'código', 'desarrollo', 'nube'];
    } else if (firstQ.startsWith('chef_')) {
      specializedKeywords = ['mermas', 'inventario', 'costo', 'receta', 'haccp', 'higiene', 'almacenamiento', 'temperatura', 'platillo', 'brigada', 'insumo', 'cocina', 'salón', 'desperdicio', 'calidad'];
    } else if (firstQ.startsWith('legal_')) {
      specializedKeywords = ['contrato', 'riesgo', 'indemnidad', 'penalidad', 'disputa', 'jurisprudencia', 'doctrina', 'recurso', 'litigio', 'mercantil', 'ley', 'norma', 'cláusula', 'tribunal', 'legal'];
    } else if (firstQ.startsWith('health_')) {
      specializedKeywords = ['paciente', 'clínico', 'diagnóstico', 'sintomatología', 'vital', 'empatía', 'acls', 'bls', 'urgencia', 'médico', 'protocolo', 'síntoma', 'clínica', 'ética', 'soporte'];
    } else if (firstQ.startsWith('admin_')) {
      specializedKeywords = ['caja', 'cash', 'liquidez', 'cobranza', 'contable', 'erp', 'sap', 'oracle', 'excel', 'presupuesto', 'conciliación', 'facturación', 'costo', 'administración', 'proceso'];
    } else if (firstQ.startsWith('sales_')) {
      specializedKeywords = ['campaña', 'roi', 'cac', 'ltv', 'conversión', 'crm', 'salesforce', 'hubspot', 'cliente', 'embudo', 'ticket', 'objeción', 'cierre', 'venta', 'marketing', 'b2b', 'b2c'];
    } else if (firstQ.startsWith('edu_')) {
      specializedKeywords = ['pedagógica', 'inclusión', 'aula', 'instruccional', 'lms', 'moodle', 'canvas', 'currículo', 'competencias', 'docencia', 'enseñanza', 'estudiante', 'evaluación', 'aprendizaje'];
    } else if (firstQ.startsWith('trade_')) {
      specializedKeywords = ['seguridad', 'epp', 'riesgo', 'diagnóstico', 'planos', 'cuadrilla', 'material', 'obra', 'protocolo', 'falla', 'avería', 'herramienta', 'maestro', 'oficio', 'plano'];
    } else {
      specializedKeywords = ['reto', 'liderazgo', 'resolución', 'priorizar', 'presión', 'plazo', 'comunicación', 'conflicto', 'equipo', 'metodología', 'planificación', 'gestión', 'solución', 'trabajo'];
    }
    ...
    if (scaleVal >= 4 && avgLen < 15) {
      score -= 20;
      incongruenceNote = 'Se detectó incongruencia heurística entre la autoevaluación muy alta y la brevedad de los argumentos técnicos proporcionados.';
    } else if (scaleVal === 5 && avgLen < 25) {
      score -= 10;
      incongruenceNote = 'Se observó una leve discrepancia entre la máxima confianza declarada y la extensión descriptiva de las respuestas prácticas.';
    }
    ...
    if (avgLen >= 40) {
      aboutMeDepth = 'Durante la evaluación de Aptly, demostró una elocuencia técnica excepcional, sustentando sus metodologías con alto rigor conceptual y planeación avanzada.';
    } else if (avgLen >= 20) {
      aboutMeDepth = 'Evidenció un enfoque pragmático orientado a objetivos directos, estructurando de forma muy coherente y solvente sus respuestas técnicas.';
    } else {
      aboutMeDepth = 'Su perfil prioriza la resolución ágil y concreta, orientándose al soporte de la continuidad operativa mediante ejecuciones precisas.';
    }

    // 3. Confidence level personalization
    if (scaleVal >= 4) {
      aboutMeConfidence = 'Muestra una sólida seguridad técnica en sus destrezas, iniciativa proactiva y capacidad natural para asumir responsabilidades de alta demanda bajo presión.';
    } else {
      aboutMeConfidence = 'Destaca por su perfil altamente colaborativo, enfocado en el trabajo armónico en equipo, el aprendizaje continuo y una comunicación asertiva sólida.';
    }
    ...
    if (keywordHits >= 6) {
      ...
      aboutMeSpecialty = 'Enfoca sus esfuerzos técnicos en el cumplimiento riguroso de normativas básicas y en la contribución eficiente a los flujos organizacionales.';
    }
