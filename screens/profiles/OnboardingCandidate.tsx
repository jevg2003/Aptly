...
export const OnboardingCandidate = ({ userId, session, onComplete }: OnboardingCandidateProps) => {
  ...
  const handleSave = async () => {
    if (!fullName || !title || !location) {
      Alert.alert(
        'Datos faltantes',
        'Por favor llena los campos obligatorios (Nombre, Título y Ubicación)'
      );
      return;
    }
        ...
        <View className="flex-1 px-6 pt-10">
          <View className="mb-8">
            ...
            <Text className="text-slate-500 dark:text-slate-400">
              Personaliza tu perfil para que las empresas puedan conocerte.
            </Text>
          </View>

          <View className="mb-8 items-center">
            <TouchableOpacity className="h-28 w-28 items-center justify-center overflow-hidden rounded-full border-2 border-dashed border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
              {initialAvatar ? (
                <Image source={{ uri: initialAvatar }} className="h-full w-full" />
              ) : (
                <>
                  <MaterialCommunityIcons name="camera-account" size={36} color="#2B468B" />
                  <Text className="mt-1 text-[10px] font-bold text-slate-500 dark:text-slate-400">
                    FOTO PERFIL
                  </Text>
                </>
              )}
            ...
            {initialAvatar ? (
              <Text className="mt-2 text-[10px] font-bold uppercase italic text-green-600 dark:text-green-400">
                Imagen Vinculada
              </Text>
            ) : null}
            <View className="mt-2 rounded-full border border-slate-200 bg-slate-100 px-3 py-1 dark:border-slate-800 dark:bg-slate-900">
          ...
          <View className="space-y-1">
            <Text className="mb-1 ml-1 font-bold text-slate-700 dark:text-slate-300">
            ...
            <Text className="mb-1 ml-1 mt-2 font-bold text-slate-700 dark:text-slate-300">
              Perfil Profesional
            </Text>
            <CustomInput
