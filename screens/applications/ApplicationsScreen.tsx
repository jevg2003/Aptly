...
export const ApplicationsScreen = ({ navigation }: any) => {
      ...
      const mapped: Application[] = (data || []).map((app) => {
        ...
        const compProf = job?.company_profile
          ? Array.isArray(job.company_profile)
            ...
            : job.company_profile
          : null;
        const candProf = app.candidate_profile
          ? Array.isArray(app.candidate_profile)
        ...
        return {
          ...
          companyName:
            role === 'company'
              ? candProf?.full_name || 'Candidato'
              : compProf?.full_name || 'Empresa',
  ...
  const filteredApplications = useMemo(() => {
    ...
    if (activeFilter === 'Finalizadas')
      return applications.filter(
        (app) =>
          app.status === 'Finalizado' || app.status === 'Rechazado' || app.status === 'Aceptado'
      );
    return applications;
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#050505' }}>
      ...
      <ObsidianHeader title="Mis Postulaciones" subtitle="Tracking" />
